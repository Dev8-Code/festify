...
import '../services/bloqueio_manual_service.dart';

final bloqueiosManuaisProvider =
    StateNotifierProvider<BloqueiosManuaisNotifier, List<DateTime>>(
...
class BloqueiosManuaisNotifier extends StateNotifier<List<DateTime>> {
  ...
  Future<void> carregar() async {
    state = await _service.buscarDatasBloqueadas();
  }
    ...
    if (isEvento) {
      if (context != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Essa é uma data de evento. Não é possível desbloquear.',
            ),
          ),
        );
      }
